...
\begin{table}[H]
	...
	\begin{tabular}{|c|}
		...
		\hline
	\end{tabular}
	\caption{Zmierzony opór zastępczy}
	\label{tab:rz}
\end{table}

% ---------- OBLICZENIA ----------
\subsection{Obliczenie oporów R1-R4}

Na podstawie pomiarów napięć i natężeń prądu dla układu z schematu 1, korzystając z prawa Ohma, obliczono wartości oporów R1 ÷ R4. Prawo Ohma wyraża się wzorem:
\begin{equation}
	R = \frac{U}{I}
\end{equation}


Wyniki obliczeń przedstawiono w tabeli \ref{tab:obliczone_opory}.

\begin{table}[h!]
	\centering
	\begin{tabular}{|c|c|}
		\hline
		Opornik & Wartość R $[\Omega]$ \\
		\hline
		$R_1$   & 189,19               \\
		$R_2$   & 2388,55              \\
		$R_3$   & 548,39               \\
		...
		\hline
	\end{tabular}
	\caption{Obliczone wartości oporów R1-R4}
	\label{tab:obliczone_opory}
\end{table}

Przykładowe obliczenie dla opornika R1:
\begin{equation}
