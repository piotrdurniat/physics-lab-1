...
Sporządzono charakterystykę prądowo-napięciową układu (rys. \ref{fig:charakterystyka_RLC}). Zgodnie z prawem Ohma dla prądu przemiennego ($I = U/Z$), współczynnik kierunkowy prostej regresji $a$ odpowiada odwrotności zawady (modułowi admitancji):
\begin{equation}
...
\begin{equation}
	u(Z) = \left| \frac{dZ}{da} \right| u(a) = \frac{1}{a^2} u(a) = Z^2 \cdot u(a)
\end{equation}

Na podstawie dopasowania prostej do danych eksperymentalnych otrzymano:
\[
	a = (1,6637 \pm 0,0035) \cdot 10^{-3}\,\mathrm{S}
\]
Stąd wyznaczona doświadczalnie wartość zawady wynosi:
\begin{equation}
	Z_{pom} = 601,07 \pm 1,27\,\Omega
\end{equation}

\subsection{Weryfikacja z modelem teoretycznym}

Wartość tę porównano z zawadą teoretyczną $Z_{teo}$, obliczoną na podstawie parametrów wyznaczonych w poprzednich częściach ćwiczenia:
\begin{itemize}
	\item Rezystancja całkowita: $R = R_{ab} + R_{bc} + R_{cd} = 85,0\,\Omega$
	\item Indukcyjność całkowita: $L = 0,9568\,\mathrm{H} \implies X_L = \omega L \approx 300,60\,\Omega$
	\item Pojemność: $C = C_1 = 3,5354\,\mu\mathrm{F} \implies X_C = \frac{1}{\omega C} \approx 900,36\,\Omega$
\end{itemize}
...
\end{equation}
\begin{equation}
	Z_{teo} = \sqrt{7225 + (-599,76)^2} \approx \sqrt{7225 + 359712} \approx 605,76\,\Omega
\end{equation}
