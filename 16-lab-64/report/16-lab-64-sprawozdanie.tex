...
Dla każdego rzędu $n \in [1, 3]$ średni kąt ugięcia $\varphi_n$ obliczony został jako połowa średniej arytmetycznej separacji kątowej między prążkami z lewej i prawej strony wiązki nieugiętej \eqref{eq:kat_ugiecia}. Różnice kątowe $|\dots|_{\text{cykl}}$ obliczono z uwzględnieniem "przejścia przez zero" na tarczy goniometru. Otrzymane wartości zapisano w tabeli \ref{tab:katy_ugiecia}.
Natomiast dla rzędu $n = 0$ kąt ugięcia przyjęto z definicji jako $\varphi_0 = 0^\circ$.

\begin{equation}
	\label{eq:kat_ugiecia}
	\varphi_n = \frac{|\varphi_{1L} - \varphi_{1P}|_{\text{cykl}} + |\varphi_{2L} - \varphi_{2P}|_{\text{cykl}}}{4}
\end{equation}
...
Stała siatki dyfrakcyjnej dla każdego rzędu $n$ obliczona została ze wzoru \eqref{eq:stala_siatki}.
...
Podstawiając wartości dla pierwszego rzędu: $d_1 \approx \SI{9.02e-6}{\meter}$, $\varphi_1 \approx \ang{4.025}$ oraz $u(\varphi) \approx \num{1.7e-4}$ rad:

\begin{equation}
	u(d_1) = \num{9.02e-6} \cdot |\ctg(\ang{4.025})| \cdot \num{1.7e-4} \approx \SI{2.2e-8}{\meter}
\end{equation}
...
Podstawiając obliczone wartości dla \( k=3 \) rzędów (gdzie \(\bar{d} \approx \SI{1.04e-5}{\meter}\)):

\begin{align*}
	 & u(\bar{d}) = \sqrt{\frac{1}{3(3-1)} \left[ (\num{9.02e-6} - \num{1.04e-5})^2 + \dots + (\num{1.15e-5} - \num{1.04e-5})^2 \right]} \\
	 & \approx \SI{7.4e-7}{\meter}
\end{align*}
...
Podstawiając wartości (\( \bar{d} \approx \SI{1.04e-5}{\meter} \), \( u(\bar{d}) \approx \SI{7.4e-7}{\meter} \), \( N \approx \SI{96}{\per\milli\meter} \)):

\begin{equation}
	u(N) \approx \SI{96}{\per\milli\meter} \cdot \frac{\num{7.4e-7}}{\num{1.04e-5}} \approx \SI{6.8}{\per\milli\meter}
\end{equation}
